...
const DocumentListPage = () => {
  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
      ...
      setUploadTitle(title.length > 50? title.substring(0, 50) : title);
    } 
  };
        ...
        <div className="relative w-full max-w-lg bg-white/95 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-2xl shadow-slate-900/20 p-8">
          ...
          <form onSubmit={handleUpload} className="space-y-5">
            ...
            <div className="space-y-2">
              ...
              required
              className="w-full h-12 px-4 border-2 border-slate-200 rounded-xl bg-slate-50/50 text-slate-900 placeholder-slate-400 text-sm font-medium transition-all duration-200 focus:outline-none focus:border-emerald-500 focus:bg-white focus:shadow-lg focus:shadow-emerald-500/10 text-ellipsis overflow-hidden"
                  ...
                  <p className="text-sm font-medium text-slate-700 mb-1">
                    {uploadFile ? (
                      <span className="text-emerald-600 truncate max-w-xs block">
                        ...
                        ? uploadFile.name.substring(0, 40) + '...' 
                        : uploadFile.name}
                      </span>
                    ) : (
                      <>
      ...
      {isDeleteModalOpen && (
        ...
        <div className="relative w-full max-w-md bg-white/95 backdrop-blur-xl border border-slate-200/60 rounded-2xl shadow-2xl shadow-slate-900/20 p-8">
          ...
          <div className="flex gap-3">
            ...
            onClick={() => setIsDeleteModalOpen(false)}
            disabled={deleting}
            className="flex-1 h-11 px-4 border-2 rounded-xl bg-white text-slate-700 text-sm font-semibold hover:bg-slate-50 hover:border-slate-300 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              Cancel
            </button>
            <button
